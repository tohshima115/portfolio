    ...
    const history = [
        {
            ...
            body: "Web サイト制作のクライアントワーク約20件をデザインから実装まで並行管理。社内ではデザイナーと実装者の橋渡し役として、「これは実装コストが重いから別案を考えよう」「ここはピクセル/パディングこう」といったデザイン⇄コードの相互翻訳と、デザイナーの環境構築・Cloudflare デプロイの伴走も担当。並行して業務改善（PL ダッシュボード / 金額入力自動化 / Schedule Distributor）を自発的に実施。",
            active: true,
        },
        {
    ...
    return (
        <div className="space-y-6 text-base text-foreground/90 leading-relaxed">
            ...
            <p className="pt-3 border-t border-border">
                ...
                というバランスが好きです。最後の2択まで絞ったらどう転んでも大きくは外れない、その手前の絞り込みをいかに理屈で詰めるか ── という型を、構図・囲碁・アーキテクチャ選定・ユーザーリサーチからの仮説立て、すべてに同じ姿勢で使い回しています。
            </p>

            <p>
                面白いのは、<strong className="text-foreground">手札（知識・パターン・データ）が増えるほど最後の感覚的判断の精度が上がっていく</strong>体感があること。技術選定でいうと、初期は手数が少なくて Next.js + Vercel を「とりあえず」で選んでいたものが、いまは React Router v7 / Astro / Hono / Cloudflare の組み合わせを根拠を持って選べる ── この精度が上がっていく経験そのものを楽しんでいます。
            </p>

            <p>
                ユーザーリサーチも同じで、インタビュー・観察を通してデータが蓄積されるほど、感覚で決める部分の精度が上がっていく。Swept の Phase 1 で「沈黙の使い方」「定性と定量の切り分け」を覚えたあと、AIChatClip の MVP で 1,000 円課金意思を即決された瞬間に「これは行ける」と判断できたのも、データの量と質の両方が手元にあったからだと思っています。
            </p>
        </div>
    );
};
