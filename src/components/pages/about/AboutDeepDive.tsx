...
const BioContent: React.FC = () => {
    return (
        <div className="space-y-6 text-base text-foreground/90 leading-relaxed">
            <p>
                キャリアの始まりは <strong className="text-foreground">デザイナー</strong> です。Illustrator / Photoshop / Figma で UI とビジュアルを作るところから入って、社会起業チーム「Swept」(3 人) でプロダクトに関わったのを境に、「自分の設計を自分でかたちにする」方向に動き出しました。
            </p>

            <p>
                ちょうど Cursor とか AI コーディングツールが出始めた時期で、当時のクオリティは荒削りでしたが「これは行ける」と感じられたのは良いタイミングだったなと思っています。Vercel + Next.js を経て、いまは
                <strong className="text-foreground"> React Router v7 + Astro + Hono + Cloudflare </strong>
                に落ち着いています。途中で Cloudflare に載せたとき、Vercel が裏で吸収してくれていた SSR / SSG / CSR の境界が見えるようになって、それが自分の理解を引き上げる訓練になったのは想定外の収穫でした（このあたりはいまも勉強中です）。
            </p>

            <p className="pt-3 border-t border-border">
                経営学部 → 起業準備 → デザイン → 個人開発、と歩いてきました。場面ごとに振っているように見えるかもしれませんが、自分のなかでは切り捨てて移った感覚はなくて、それぞれで身についたものが今の個人開発で道具として残っているなあと感じています。
            </p>

            <p>
                経営学部で身についた事業視点、起業準備 2 年で身についた検証の習慣（
                <strong className="text-foreground">リーンスタートアップの考え方、ユーザーインタビューの度胸、小さく検証する手癖、上流から「それ本当に必要？」と問い直す目線</strong>
                ）、デザインで身についた設計の理論 ── どれも今の個人開発のなかで使っています（デザインは引き続き勉強中です）。
            </p>

            <p>
                ものづくりの専門性を深めるには長い時間がかかるので、次の場所は腰を据えて長く関われるところを選びたいと思っています。
            </p>

            <p className="pt-3 border-t border-border">
                プロダクトの判断は、技術的な美しさよりも事業から逆算する側に寄っています。AIChatClip は MVP の前に
                <strong className="text-foreground">1,000 円の課金意思を実際に確認してから</strong>
                着手しましたし、デザイン事務所の PL ダッシュボードも先輩の業務を見て自発的に提案して作ったり、というのが性格として一貫しているなと自覚しています。
            </p>

            <p>
                デザインの方は <strong className="text-foreground">Material Design 2 / 3 を基盤として読み込んでいる</strong>
                タイプで、角丸の値や余白、コンポーネント設計に都度根拠を持てる程度には理論で組んでいます ── 雰囲気だけでデザインしているわけではない、というのは一応言わせてください。
            </p>

            <p>
                動き方は UI / UX 寄りで、AIChatClip のオンボーディングを IT が苦手な友人に試してもらって、どこで詰まるかを観察する、みたいなユーザーテストっぽい工程をルーティンに組み込んでいます。グラフィックを作るデザイナーというより、設計寄り・UI / UX 寄りのデザイナーです。
            </p>
        </div>
    );
};
