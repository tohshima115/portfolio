            ...
            <section>
                ...
                <p>
                    ...
                    <strong className="text-foreground">
                        2025 年夏過ぎから本番採用しはじめて、いまはほぼ手癖で選んでいる
                    </strong>
                    感覚です。
                </p>
                <p className="mt-3">
                    「優れているから採用」というより、個人開発者・スタートアップにここまで優しいサービスは他に無いと感じていて、SQLite ベースの D1 が React Router v7 / Astro と素直に噛み合うこと、Vercel のように裏でよしなにやってくれる代わりに
                    <strong className="text-foreground">
                        「なぜ動かないか / なぜ動くか」を自分で追える難易度
                    </strong>
                    であることが性に合っています。SSR / SSG / CSR の境界は最初は何度も詰まりましたが、Cloudflare に載せたことで Vercel が裏で吸収してくれていた区別が明確に必要になり、結果として自分の理解を引き上げる訓練になりました（ここは現在進行で勉強中です）。
                </p>
                <p className="mt-3">
    ...
    return (
        <div className="space-y-6 text-base text-foreground/90 leading-relaxed">
            <p>
                ...
                着手、PL ダッシュボードも先輩の業務を見て自発的に提案・制作、と性格として一貫しています。
            </p>
            ...
            <p>
                ...
                <strong className="text-foreground">「自分の設計を自分でかたちにする」</strong>
                方向に舵を切りました。
            </p>
            ...
            <p>
                ...
                に落ち着くまでフレームワーク選定を実験しながらフロント〜バックを覚えていきました。途中で Cloudflare に載せたとき、Vercel が裏でよしなにやっていた SSR / SSG / CSR の境界が急に見えるようになり、それが自分の理解を一段引き上げる訓練になったのは想定外の収穫でした。
            </p>

            <p>
                少人数チームで事業を回す経験を起点に、AI 進化で「1人で完結できる」幅が広がっていく流れにそのまま乗って、いまは個人開発（AIChatClip）に重心を置いています。
                <strong className="text-foreground">
                    「ガッツリのエンジニア」を演じるのではなく、デザイナー出身でものづくりの最後まで担える人間として勝負したい
                </strong>
                ──というのが立ち位置です。
            </p>
            ...
            <p className="pt-3 border-t border-border">
                ...
                <strong className="text-foreground">
                    「諦めずに続けてきたものが、次のステージでどこまで自分の武器になるかを試す」
                </strong>
                こと。経営学部 → 起業準備 → デザイン → 個人開発、と歩いてきましたが、それぞれを切り捨てて移ってきた感覚はなく、経営学部で身についた事業視点・起業準備 2 年で身についた検証の習慣・デザインで身についた設計の理論 ── どれも今の個人開発のなかに道具として積み上がっています（デザインは引き続き勉強中です）。
                ものづくりの専門性を深めるには長い時間がかかると分かっているので、次の場所は腰を据えて長く関われる場所として選ぼうとしています。
            </p>

            <p>
                起業準備の 2 年間で具体的に身についたのは、
                <strong className="text-foreground">
                    リーンスタートアップの考え方、ユーザーインタビューの度胸、小さいスパンで検証することへの抵抗のなさ、検証に時間をかけることを厭わない感覚
                </strong>
                ── そして「それは本当に必要なのか」を上流から問い直す経営者視点。頭でっかちだった自分が、まず動いてみる側に振り切れた 2 年でした。
            </p>

            <p>
                デザインは <strong className="text-foreground">Material Design 2 / 3 を基盤として読み込んでいる</strong>タイプで、角丸の値や余白・コンポーネントの設計に都度根拠を持てる程度には理論で組みます。雰囲気でデザインしているわけではない、というのはここで一応強調させてください。
            </p>

            <p>
                動き方としては UI / UX 寄りのスタンスで、AIChatClip のオンボーディングを IT が苦手な友人に試してもらってどこで詰まるかを観察する、ユーザーテストのような工程をルーティンに組み込んでいます。グラフィックデザイナーというより、設計寄り・UI / UX 寄りのデザイナーです。
            </p>
        </div>
    );
};
