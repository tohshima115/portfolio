...
import { R as ROLES } from '../../../chunks/consts_DRUuioct.mjs';
...
const config = config$1({
  ...
  collections: {
    projects: collection({
      ...
      // Removed trailing slash for file mode
      format: { contentField: "content" },
      // Maps MDX body to 'content' field
      schema: {
        title: index.slug({ name: { label: "プロジェクト名" } }),
        // 1. メタデータ
        meta: index.object({
          ...
          date: index.date({ label: "プロジェクト完了年月" }),
          updatedDate: index.date({ label: "最終更新日", description: "更新一覧に表示される日時" }),
          duration: index.text({ label: "制作期間 (例: 3ヶ月)" }),
          link: index.url({ label: "プロジェクトURL (任意)" })
        }, { label: "基本情報" }),
        // 2. タグ・属性
        attributes: index.object({
          roles: index.multiselect({
            label: "担当した役割",
            options: ROLES
          }),
          stack: index.array(
            index.text({ label: "技術・ツール (例: Figma, React)" }),
