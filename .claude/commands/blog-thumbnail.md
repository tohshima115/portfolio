...
7. 選んだ1枚をそのまま `src/assets/blog/<slug>.png` に保存する（もう1枚は破棄してよい）。ノイズが残っている場合のみ `scripts/clean-thumbnail.py` を使う（上記「ノイズ対策」参照）
8. `python scripts/generate-thumbnail-formats.py <slug>` を実行し、`public/blog/<slug>.{avif,webp,png}` を生成する（サムネイルは`<picture>`でAVIF優先→WebPフォールバック→PNGフォールバックで配信する構成になっている。詳細は「サムネイルの配信方式」参照）
9. 対象記事の frontmatter に `thumbnail: <slug>` を追加する（パスではなくbasenameのみ）
10. コミットする（メッセージ例：`feat(blog): 〇〇記事のサムネイルを追加`）
